...
observed_diabetes_distance = np.count_nonzero(framingham.column('DIABETES'))/framingham.num_rows
...
def diabetes_statistic():
    sim_prop = sample_proportions(framingham.num_rows, diabetes_proportions)
    return abs(sim_prop[1] - observed_diabetes_distance)
...
# Yes, we should reject the null hypothesis because the data is consistent with the alternative hypothesis. 
...
framingham_diabetes_possibilities = make_array(1, 2)
...
# The test statistic would be the difference of means between cholesterol levels of people with heart disease and without heart disease. The order will be: those with heart disease - those without heart disease. Because of this, if people with heart disease have high cholesterol levels, the mean of those people would be larger, making the difference larger. If they come from the same distribution, the means would be similar causing the difference to be smaller. 
...
def compute_framingham_test_statistic(tbl):
    with_hd = sum(tbl.where('ANYCHD', are.equal_to(1)).column('TOTCHOL'))/tbl.where('ANYCHD', are.equal_to(1)).num_rows
    without_hd = sum(tbl.where('ANYCHD', are.equal_to(0)).column('TOTCHOL'))/tbl.where('ANYCHD', are.equal_to(0)).num_rows
...
def simulate_framingham_null():
    ...
    original_and_shuffled = framingham.drop('ANYCHD').with_column('ANYCHD', shuffled_labels)
    shuffled_means = compute_framingham_test_statistic(original_and_shuffled.select('TOTCHOL', 'ANYCHD'))
...
# One reason is because it is an observational study, rather than a randomized controlled experiment. They could not randomly assign heart disease to a person, and this leads to the possibility that other causes account for the high cholesteral levels. 
...
event_result = make_array('N', 'S', 'L', 'L', 'S') #not sure
event_result
...
def relative_risk(tbl):
    ...
    grouped_tbl = tbl.group(['HRT', 'Heart Attack'])
    ones_only = grouped_tbl.where('HRT', are.equal_to(1)).column('count')
    numerator = ones_only.item(1) / ones_only.item(0)
    zeros_only = grouped_tbl.where('HRT', are.equal_to(0)).column('count')
    denominator = zeros_only.item(1) / zeros_only.item(0)
...
# From this test, we can conclude that hormone replacement therapy is associated with a decreased risk of heart attacks because the confidence interval of the test statistic does not include 1. We cannot say that HRT  caused a changeg in heart attack risk because this was an observational study and there could have been confounding factors that caused certain nurses to be more or less inherent to the risk. 
...
def HERS_test_statistic(tbl):
    ...
    averages_1 = np.sum(tbl.where('HRT', 1).column('CHD'))/len(tbl.where('HRT', 1).column('CHD'))
    averages_2 = np.sum(tbl.where('HRT', 0).column('CHD'))/len(tbl.where('HRT', 0).column('CHD'))
    return abs(averages_1 - averages_2)
...
def simulate_one_HERS_statistic():
    ...
    new_tbl = HERS.drop('HRT').with_column('HRT', shuffled)
    return HERS_test_statistic(new_tbl)
...
HERS_pval = np.sum(HERS_test_statistics <= observed_HERS_test_statistic)/len(HERS_test_statistics)
HERS_pval
...
# Since our calculated p-value of 18% is much larger than the cutoff, we can accept the null hypothesis because the evidence suggests that the difference between the averages is small enough to assume CHD risk is due to chance. 
...
# It would bias the results because the subjects in the HRT group are systematically healthier and less prone to CHD due to something other than the HRT treatment. Them being healthy would consistently show results that suggest their group is less prone to CHD, and we would expect the observational study to overestimate the protective effect of HRT. 
...
# One ethical problem is how Frantz treated consent. Patients that either have a disability or a mental illness might not be able to consent to the study physically, or might not be fully aware of the experiment regardless of "the opportunity to decline participation". Using these patients as test subjects is unfair to them. 
...
# Null hypothesis: An unsaturated fat diet does not affect death rate and any association is due to chance. Alternative hypothesis: The unsaturated fat diet has a significant effect on death rate. 
...
# One reason why it is not sufficient is because people in the mental hospital were already experiencing some kind of health issue and were more prone to dying than if the subjects were chosen at random. So, the deaths could be accounted by reasons other than their diet. 
...
def compute_hazard_difference(tbl):
    return np.abs(tbl.column('Hazard Rate').item(0) - tbl.column('Hazard Rate').item(1))
...
def complete_test(t):
    def one_sim(t):
        shuffled_col = t.sample(with_replacement = False).column('Condition')
        shuffled_tab = t.drop('Condition').with_column('Condition', shuffled_col)
        grouped_tab = shuffled_tab.group("Condition", np.sum).drop('Age sum')
        haz_tab = grouped_tab.with_column('Hazard Rate', grouped_tab.column('Died sum') / grouped_tab.column('Participated sum'))
        return compute_hazard_difference(haz_tab)
    sim_differences = make_array()
    for i in np.arange(500):
        one_diff = one_sim(t)
        sim_differences = np.append(sim_differences, one_diff)
    p_val = np.count_nonzero(sim_differences >= death_rate_observed_statistic) / len(sim_differences)
    return p_val
